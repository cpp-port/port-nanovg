...
static void glnvg__renderFlush(void* uptr)
{
	...
	if (gl->ncalls > 0) {
		...
		// Upload vertex data
#if defined NANOVG_GL3
		// Vertex array objects are container objects and are not shared between
		// OpenGL contexts. NanoVG can be flushed by another context in the same
		// share group, so use a VAO owned by the context performing this flush.
